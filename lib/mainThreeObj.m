...
%% Load image data
for i = 1:size(threeObj.Files,1)
disp(['Calculating object-', num2str(i)]);
[img, imgInfo] = readimage(threeObj, i);
...
for numObj = 1:size(obj,2)
    ...
    saveas(gca, fullfile('../data/figure/normals/3', strcat(num2str(i), '.jpg')));
end
...
saveas(gca, fullfile('../data/figure/light/3', strcat(num2str(i), '.jpg')));
...
writetable(tblInfo, strcat('../data/lightInfo3Object-',num2str(threshold),'.xlsx'));